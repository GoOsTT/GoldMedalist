...
const numberGengerMedalist = (gender, country) => {
  if (['Men', "Women"].includes(gender)) {
    return `SELECT COUNT(DISTINCT name) FROM GoldMedal WHERE gender = '${gender}' AND country = '${country}';`
  }
}

const numberMenMedalists = country => {
  return numberGengerMedalist('Men', country);
};
...
const numberWomenMedalists = country => {
  return numberGengerMedalist('Women', country);
};
...
const orderedSports = (country, field, sortAscending) => {
  ...
  return `SELECT sport, COUNT(sport) AS count, (COUNT(sport) * 100 / (select COUNT(*) 
          FROM GoldMedal 
          WHERE country = '${country}')) AS percent FROM GoldMedal WHERE country = '${country}' GROUP BY sport ${orderingString};`;
};
